...
- place unit test files in the same directory as the files they test
...
- instead of `const questions = cards.map(c => c.question).sort()` , use `const questions = cards.map(c => c.question).sort((a, b) => a.localeCompare(b))`
...
pnpm --filter 1x1 vitest src/services/__tests__/storage.test.ts
